...
lcMesh* lcLibraryMeshData::CreateMesh()
{
	...
	for (const lcMeshLoaderTypeData& Data : mData)
	{
		for (const lcMeshLoaderVertex& SrcVertex : Data.mVertices)
		{
			...
			DstVertex.Position = lcVector3LDrawToSECAD(SrcVertex.Position);
			DstVertex.Normal = lcPackNormal(lcVector3LDrawToSECAD(SrcVertex.Normal));
		}
	}
	...
	for (const lcMeshLoaderTypeData& Data : mData)
	{
		for (const lcMeshLoaderConditionalVertex& SrcVertex : Data.mConditionalVertices)
		{
			...
			DstVertex.Position1 = lcVector3LDrawToSECAD(SrcVertex.Position[0]);
			DstVertex.Position2 = lcVector3LDrawToSECAD(SrcVertex.Position[1]);
			DstVertex.Position3 = lcVector3LDrawToSECAD(SrcVertex.Position[2]);
			DstVertex.Position4 = lcVector3LDrawToSECAD(SrcVertex.Position[3]);
		}
	}

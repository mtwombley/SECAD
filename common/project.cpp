...
bool Project::Load(const QString& FileName, bool ShowErrors)
{
	...
	if (Extension == QLatin1String("dat") || Extension == QLatin1String("ldr") || Extension == QLatin1String("mpd"))
		LoadDAT = true;
	else if (Extension == QLatin1String("lcd") || Extension == QLatin1String("SECAD"))
		LoadDAT = false;
	else
		LoadDAT = FileData.size() > 7 && memcmp(FileData, "SECAD ", 7);
...
bool Project::Export3DStudio(const QString& FileName)
{
	...
	if (ModelParts.empty())
	{
		QMessageBox::information(gMainWindow, tr("SECAD"), tr("Nothing to export."));
		return false;
	}
	...
	if (!File.Open(QIODevice::WriteOnly))
	{
		QMessageBox::warning(gMainWindow, tr("SECAD"), tr("Could not open file '%1' for writing.").arg(SaveFileName));
		return false;
	}
...
bool Project::ExportCOLLADA(const QString& FileName)
{
	...
	Stream << "<unit name=\"SECAD\" meter=\"0.0004\" />\r\n";
...
void Project::ExportHTML(const lcHTMLExportOptions& Options)
{
	...
	for (lcModel* Model : Models)
	{
		...
		if (Options.SinglePage)
		{
			...
			Stream << QLatin1String("</CENTER>\r\n<BR><HR><BR><B><I>Created by <A HREF=\"https://www.SECAD.org\">SECAD</A></I></B><BR></HTML>\r\n");
		}
		else
		{
			if (Options.IndexPage)
			{
				...
				Stream << QLatin1String("</CENTER>\r\n<BR><HR><BR><B><I>Created by <A HREF=\"https://www.SECAD.org\">SECAD</A></B></I><BR></HTML>\r\n");
			}
...
bool Project::ExportPOVRay(const QString& FileName)
{
	...
	sprintf(Line, "// Generated By: SECAD %s\n// LDraw File: %s\n// Date: %s\n\n",
			LC_VERSION_TEXT,
	...
	if (UseLGEO)
	{
		...
		{
			QMessageBox::information(gMainWindow, tr("SECAD"), tr("Could not find LGEO files in folder '%1'.").arg(LGEOPath));
			return false;
		}
...
bool Project::ExportWavefront(const QString& FileName)
{
	...
	OBJFile.WriteLine("# Model exported from SECAD\n");
	...
	{
		QMessageBox::warning(gMainWindow, tr("SECAD"), tr("Could not open file '%1' for writing.").arg(MaterialFileName));
		return false;
	}

	MaterialFile.WriteLine("# Colors used by SECAD\n\n");
	for (const lcColor& Color : gColorList)
	{

...
{
	setApplicationDisplayName(QLatin1String("SECAD"));
...
void lcApplication::ExportClipboard(const QByteArray& Clipboard)
{
	...
	MimeData->setData("application/vnd.SECAD-clipboard", Clipboard);
	...
	while (!Arguments.isEmpty())
	{
		...
		else if (Option == QLatin1String("--camera-position") || Option == QLatin1String("--camera-position-ldraw"))
		{
			if ((Options.SetCameraPosition = ParseFloatArray(9, Options.CameraPosition[0], true)))
			{
				...
				{
					Options.CameraPosition[0] = lcVector3LDrawToSECAD(Options.CameraPosition[0]);
					Options.CameraPosition[1] = lcVector3LDrawToSECAD(Options.CameraPosition[1]);
					Options.CameraPosition[2] = lcVector3LDrawToSECAD(Options.CameraPosition[2]);
				}
			}
		}
		...
		else if (Option == QLatin1String("-v") || Option == QLatin1String("--version"))
		{
			...
			Options.StdOut += tr("SECAD Continuous Build %1\n").arg(QT_STRINGIFY(LC_CONTINUOUS_BUILD));
#else
			Options.StdOut += tr("SECAD Version %1\n").arg(LC_VERSION_TEXT);
#endif
		...
		{
			Options.StdOut += tr("Usage: SECAD [options] [file]\n");
...
lcStartupMode lcApplication::Initialize(const QList<QPair<QString, bool>>& LibraryPaths)
{
	...
	if (!LoadPartsLibrary(Options.LibraryPaths.isEmpty() ? LibraryPaths : Options.LibraryPaths, !Options.LibraryPaths.isEmpty()))
	{
		...
		if (mLibrary->LoadBuiltinPieces())
			Message = tr("SECAD could not find a compatible Parts Library so only a small number of parts will be available.\n\nPlease visit https://www.SECAD.org for information on how to download and install a library.");
		else
			Message = tr("SECAD could not load Parts Library.\n\nPlease visit https://www.SECAD.org for information on how to download and install a library.");

		if (gMainWindow)
			QMessageBox::information(gMainWindow, tr("SECAD"), Message);
		else
		{
			StdErr << Message << "\n";
			StdErr.flush();
...
void lcApplication::ShowPreferencesDialog()
{
	...
	if (LanguageChanged || LibraryChanged || AAChanged)
		QMessageBox::information(gMainWindow, tr("SECAD"), tr("Some changes will only take effect the next time you start SECAD."));

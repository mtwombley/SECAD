...
{
	setAcceptDrops(true);
	setWindowIcon(QIcon(":/resources/SECAD.png"));
...
{
	const QString MimeType = QLatin1String("application/vnd.SECAD-clipboard");
...
void lcMainWindow::MergeProject()
{
	...
	if (NewProject->Load(LoadFileName, true))
	{
		...
		if (NumModels == 1)
			QMessageBox::information(this, tr("SECAD"), tr("Merged 1 submodel."));
		else
			QMessageBox::information(this, tr("SECAD"), tr("Merged %1 submodels.").arg(NumModels));
...
void lcMainWindow::HandleCommand(lcCommandId CommandId)
{
	...
	case LC_HELP_HOMEPAGE:
		QDesktopServices::openUrl(QUrl("https://www.SECAD.org/"));
		break;

	case LC_HELP_BUG_REPORT:
		QDesktopServices::openUrl(QUrl("https://github.com/leozide/SECAD/issues"));
		break;

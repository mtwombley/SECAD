...
void lcModelProperties::SaveLDraw(QTextStream& Stream) const
{
	...
	if (!mComments.isEmpty())
	{
		...
		for (const QString& Comment : Comments)
			Stream << QLatin1String("0 !SECAD MODEL COMMENT ") << Comment << LineEnding;
	}
...
void lcPOVRayOptions::SaveLDraw(QTextStream& Stream) const
{
	...
	if (!HeaderIncludeFile.isEmpty())
		Stream << QLatin1String("0 !SECAD POV_RAY HEADER_INCLUDE_FILE ") << QDir::toNativeSeparators(HeaderIncludeFile) << LineEnding;
	if (!FooterIncludeFile.isEmpty())
		Stream << QLatin1String("0 !SECAD POV_RAY FOOTER_INCLUDE_FILE ") << QDir::toNativeSeparators(FooterIncludeFile) << LineEnding;
	if (FloorAxis != 1)
		Stream << QLatin1String("0 !SECAD POV_RAY FLOOR_AXIS ") << FloorAxis << LineEnding;
	if (FloorColor != lcVector3(0.8f,0.8f,0.8f))
		Stream << QLatin1String("0 !SECAD POV_RAY FLOOR_COLOR_RGB ") << FloorColor[0] << ' ' << FloorColor[1] << ' ' << FloorColor[2] << LineEnding;
	if (FloorAmbient != 0.4f)
		Stream << QLatin1String("0 !SECAD POV_RAY FLOOR_AMBIENT ") << FloorAmbient << LineEnding;
	if (FloorDiffuse != 0.4f)
		Stream << QLatin1String("0 !SECAD POV_RAY FLOOR_DIFFUSE ") << FloorDiffuse << LineEnding;
	if (ExcludeFloor)
		Stream << QLatin1String("0 !SECAD POV_RAY EXCLUDE_FLOOR") << LineEnding;
	if (ExcludeBackground)
		Stream << QLatin1String("0 !SECAD POV_RAY EXCLUDE_BACKGROUND") << LineEnding;
	if (NoReflection)
		Stream << QLatin1String("0 !SECAD POV_RAY NO_REFLECTION") << LineEnding;
	if (NoShadow)
		Stream << QLatin1String("0 !SECAD POV_RAY NO_SHADOWS") << LineEnding;
	if (UseLGEO)
		Stream << QLatin1String("0 !SECAD POV_RAY USE_LGEO") << LineEnding;
}
...
void lcModel::SaveLDraw(QTextStream& Stream, bool SelectedOnly, lcStep LastStep) const
{
	...
	for (lcPiece* Piece : mPieces)
	{
			...
			if (CurrentGroups.IsEmpty() || (!CurrentGroups.IsEmpty() && PieceGroup != CurrentGroups[CurrentGroups.GetSize() - 1]))
			{
				...
				while (!CurrentGroups.IsEmpty())
				{
					...
					if (Index == -1)
					{
						...
						Stream << QLatin1String("0 !SECAD GROUP END\r\n");
					}
					else
					{
						FoundParent = Index;
				...
				for (int ParentIdx = FoundParent + 1; ParentIdx < PieceParents.GetSize(); ParentIdx++)
				{
					...
					Stream << QLatin1String("0 !SECAD GROUP BEGIN ") << Group->mName << LineEnding;
				}
			}
		}
		else
		...
		{
			Stream << QLatin1String("0 !SECAD SYNTH BEGIN\r\n");
			...
			for (int ControlPointIdx = 0; ControlPointIdx < ControlPoints.GetSize(); ControlPointIdx++)
			{
				...
				Stream << QLatin1String("0 !SECAD SYNTH CONTROL_POINT");
		...
		if (Piece->mPieceInfo->GetSynthInfo())
			Stream << QLatin1String("0 !SECAD SYNTH END\r\n");
	}
...
QImage lcModel::GetStepImage(bool Zoom, int Width, int Height, lcStep Step)
{
	...
	{
		QMessageBox::warning(gMainWindow, tr("SECAD"), tr("Error creating images."));
		return QImage();
	}
...
void lcModel::GroupSelection()
{
	...
	{
		QMessageBox::information(gMainWindow, tr("SECAD"), tr("No pieces selected."));
		return;
	}
...
void lcModel::InlineSelectedModels()
{
	...
	{
		QMessageBox::information(gMainWindow, tr("SECAD"), tr("No models selected."));
		return;
	}
...
void lcModel::ShowSelectByNameDialog()
{
	...
	{
		QMessageBox::information(gMainWindow, tr("SECAD"), tr("Nothing to select."));
		return;
	}
...
void lcModel::ShowArrayDialog()
{
	...
	{
		QMessageBox::information(gMainWindow, tr("SECAD"), tr("Array only has 1 element or less, no pieces added."));
		return;
	}

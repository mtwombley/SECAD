...
void lcPiece::SaveLDraw(QTextStream& Stream) const
{
	...
	if (mStepHide != LC_STEP_MAX)
		Stream << QLatin1String("0 !SECAD PIECE STEP_HIDE ") << mStepHide << LineEnding;

	if (IsHidden())
		Stream << QLatin1String("0 !SECAD PIECE HIDDEN") << LineEnding;
	...
	if (mPivotPointValid)
	{
		...
		Stream << QLatin1String("0 !SECAD PIECE PIVOT ");

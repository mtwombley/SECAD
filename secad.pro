...
lessThan(QT_MAJOR_VERSION, 5) {
	error("SECAD requires Qt5.4 or later.")
}
...
win32 {
	RC_ICONS = resources/secad.ico
	VERSION = 23.3.0.0
	QMAKE_TARGET_COMPANY = SECAD Software
	QMAKE_TARGET_DESCRIPTION = SECAD
	QMAKE_TARGET_COPYRIGHT = "Copyright (C) SECAD.org"
	QMAKE_TARGET_PRODUCT = SECAD
	RC_CODEPAGE = 1252
	RC_LANG = 0x0409
...
!isEmpty(APPVEYOR_REPO_COMMIT) {
	isEmpty(APPVEYOR_REPO_TAG) {
		DEFINES += "LC_CONTINUOUS_BUILD=$$system(git rev-parse --short HEAD)"
		VERSION = 0.0.0.1
		QMAKE_TARGET_DESCRIPTION = "SECAD Continuous Build $$system(git rev-parse --short HEAD)"
	}
}

TSFILES = resources/secad_fr.ts resources/secad_pt.ts resources/secad_de.ts resources/secad_uk.ts resources/secad_cs.ts resources/secad_es.ts
...
unix:!macx {
	...
	isEmpty(DOCS_DIR):DOCS_DIR = $$INSTALL_PREFIX/share/doc/secad
	...
	man.path = $$MAN_DIR
	man.files = docs/secad.1
	desktop.path = $$DESKTOP_DIR
	desktop.files = qt/secad.desktop
	...
	mime.path = $$MIME_DIR
	mime.files = qt/secad.xml
	appdata.path = $$APPDATA_DIR
	appdata.files = tools/setup/secad.appdata.xml
...
macx {
	ICON = resources/secad.icns
	QMAKE_INFO_PLIST = qt/Info.plist

	document_icon.files += $$_PRO_FILE_PWD_/resources/secad_document.icns
...
OTHER_FILES +=
RESOURCES += secad.qrc resources/stylesheet/stylesheet.qrc

!win32 {
	TRANSLATIONS = resources/secad_pt.ts resources/secad_fr.ts resources/secad_de.ts resources/secad_uk.ts resources/secad_cs.ts resources/secad_es.ts
}

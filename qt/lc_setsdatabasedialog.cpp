    ...
    ui(new Ui::lcSetsDatabaseDialog)
{
	...
	mKeyListReply = mHttpManager->DownloadFile(QLatin1String("https://www.SECAD.org/rebrickable.json"));
}
...
void lcSetsDatabaseDialog::accept()
{
	...
	if (!Current)
	{
		QMessageBox::information(this, "SECAD", tr("Please select a set from the list."));
		return;
	}
...
void lcSetsDatabaseDialog::on_SearchButton_clicked()
{
	...
	if (Keyword.isEmpty())
	{
		QMessageBox::information(this, "SECAD", tr("Keyword cannot be empty."));
		return;
	}
...
void lcSetsDatabaseDialog::DownloadFinished(lcHttpReply* Reply)
{
	if (Reply == mKeyListReply)
	{
		...
		if (mKeys.isEmpty())
		{
			QMessageBox::information(this, "SECAD", tr("Error connecting to server."));
			close();
		}
	...
	else if (Reply == mInventoryReply)
	{
		if (!Reply->error())
			...
			QMessageBox::information(this, "SECAD", tr("Error downloading set inventory."));

...
{ 
	...
	if (writeDump)
	{
		...
		lstrcpy(message, TEXT("SECAD just crashed. Crash information was saved to the file '"));
		lstrcat(message, gMinidumpPath);
		lstrcat(message, TEXT("', please send it to the developers for debugging."));

		MessageBox(nullptr, message, TEXT("SECAD"), MB_OK);
	}
...
static void lcSehInit()
{
	if (GetTempPath(LC_ARRAY_COUNT(gMinidumpPath), gMinidumpPath))
		lstrcat(gMinidumpPath, TEXT("SECAD.dmp"));
...
static void lcRegisterShellFileTypes()
{
	...
	if (RegSetValue(HKEY_CLASSES_ROOT, TEXT("SECAD.Project"), REG_SZ, TEXT("SECAD Project"), lstrlen(TEXT("SECAD Project")) * sizeof(TCHAR)) != ERROR_SUCCESS)
		return;

	lstrcpy(temp, modulePath);
	lstrcat(temp, TEXT(",0"));
	if (RegSetValue(HKEY_CLASSES_ROOT, TEXT("SECAD.Project\\DefaultIcon"), REG_SZ, temp, lstrlen(temp) * sizeof(TCHAR)) != ERROR_SUCCESS)
		return;

	lstrcpy(temp, modulePath);
	lstrcat(temp, TEXT(" \"%1\""));
	if (RegSetValue(HKEY_CLASSES_ROOT, TEXT("SECAD.Project\\shell\\open\\command"), REG_SZ, temp, lstrlen(temp) * sizeof(TCHAR)) != ERROR_SUCCESS)
		return;

	LONG size = 2 * _MAX_PATH;
	LONG result = RegQueryValue(HKEY_CLASSES_ROOT, TEXT(".lcd"), temp, &size);

	if (result != ERROR_SUCCESS || !lstrlen(temp) || lstrcmp(temp, TEXT("SECAD.Project")))
	{
		if (RegSetValue(HKEY_CLASSES_ROOT, TEXT(".lcd"), REG_SZ, TEXT("SECAD.Project"), lstrlen(TEXT("SECAD.Project")) * sizeof(TCHAR)) != ERROR_SUCCESS)
			return;
...
{
	QCoreApplication::setOrganizationDomain(QLatin1String("SECAD.org"));
	QCoreApplication::setOrganizationName(QLatin1String("SECAD Software"));
	QCoreApplication::setApplicationName(QLatin1String("SECAD"));
	...
	if (Translator.load("SECAD_" + Locale.name(), ":/resources"))
...
#ifdef Q_OS_LINUX
	LibraryPaths += qMakePair(QDir::cleanPath(QCoreApplication::applicationDirPath() + "/../share/SECAD/library.bin"), true);
#endif
